 ...
 */
package com.example.billmanager.common.aop;
...
public class RestControllerAspect {
    @Pointcut("execution(* com.example.billmanager.*.controller.*.*(..))")
    public void endPoint() {
    }

    @Before("endPoint()")
    public void before(JoinPoint joinpoint) {
        System.out.println(String.format("接口[%s]请求入参:[%s]",joinpoint, Arrays.toString(joinpoint.getArgs())));
    }

    @AfterReturning(value = "endPoint()", returning = "result")
    public void after(JoinPoint joinpoint, Object result) {
        if (result != null) {
            System.out.println(String.format("接口[%s]返回结果:[%s]",joinpoint, result.toString()));
        }
    }
